...
<?php
$varmi = rows("SELECT * FROM icerikler WHERE icerik_kategori='{$kategori_id}' && icerik_onay=1", $db);
if($varmi>0){
$sayfa = g("s") ? g("s") : 1;
$ksayisi = rows("SELECT * FROM icerikler WHERE icerik_kategori='{$kategori_id}' && icerik_onay=1", $db);
$limit = 9;
...
?>
<div class="row">
<div class="col-lg-12"><h2 class="page-header"><i class="fa fa-folder-open"></i> <?php echo $kategori['kategori_baslik']; ?> <small>Sayfa <?php echo $sayfa; ?></small></h2></div>
</div>
<div class="row">
<?php
foreach(row("SELECT * FROM icerikler INNER JOIN kategoriler ON kategoriler.kategori_id=icerikler.icerik_kategori WHERE icerik_kategori='{$kategori_id}' && icerik_onay=1 ORDER BY icerik_id DESC LIMIT $baslangic, $limit", $db) as $kayit){
?>
...
<i class="fa fa-warning"></i><strong> Bu kategoriye hiç içerik eklenmemiş.</strong>
</div>
</div>
</div>
<?php } ?>
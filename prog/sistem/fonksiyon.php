	...
	function seo($str, $options = array())
{
	...
	function row($query, $db){
		return $db->query($query, PDO::FETCH_ASSOC);
	}
	
	function rows($query, $db){
	...
	function tarih($tarihim){
		...
		return strftime("%d %B %Y - %H:%M",strtotime($tarihim));
		// return $tarihim = date("d-M-Y",strtotime($tarihim));
	}
	
	function tarih_saatsiz($tarihim){
		@setlocale(LC_ALL, 'turkish');
		return strftime("%d %B %Y",strtotime($tarihim));
	}
	
	function sayi_format($sayi){
	...
	function kisalt($kelime, $str = 10)
   {
	...
	function url($url){
		...
		{
			return TEMA_URL."/img/no-img.png";
		}
	}
?>